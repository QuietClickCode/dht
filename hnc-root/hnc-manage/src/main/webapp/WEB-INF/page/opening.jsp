    ...
    var treeColumns=[
        ...
        },
        {
            ...
            formatter:function(value,row,index){
                ...
                if(now<new Date(row.ostartTime).pattern('yyyy-MM-dd')){
                    html='未开始';
                }
                if(now>=new Date(row.ostartTime).pattern('yyyy-MM-dd') && now<=new Date(row.oendTime).pattern('yyyy-MM-dd')){
                    html='进行中';
                }
                if(now>new Date(row.oendTime).pattern('yyyy-MM-dd')){
                    html='已结束';
                }
                return html;
            }
