(function ($) {
    ...
    var getChild = function (node, source, field,treePid) {
        var data = [];
        var items = $.grep(source, function (item, index) {
            return item[treePid] == node[field];
        });
        $.each(items, function (index, item) {
            let parLevel = myMap.get(item[treePid]);
            item.level=parLevel+1;
            myMap.set(item[field],parLevel+1);
            data.push(item);
            var child = getChild(item, source, field,treePid);
            $.each(child, function (i, n) {
                let parLevel = myMap.get(n[treePid]);
                myMap.set(n[field],parLevel+1);
                n.level=parLevel+1;
                data.push(n);
            });
        });
